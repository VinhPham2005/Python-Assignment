...
def get_statistic(target_statistic, target_rows):
    target = []
    for row in target_rows:
        try:
            ...
            continue
        target_data = {}
        for statistic, statistic_id in target_statistic.items():
            target_data[statistic] = row.find_element(By.CSS_SELECTOR, f'td[data-stat="{statistic_id}"]').text.strip()
...
for row in rows:
    player_data = {}
    for statistic, statistic_id in player_statistic.items():
        player_data[statistic] = row.find_element(By.CSS_SELECTOR, f'td[data-stat="{statistic_id}"]').text.strip()
...
targets = [
    ('table#stats_keeper',  gk_statistic, 'https://fbref.com/en/comps/9/keepers/Premier-League-Stats'),
    ('table#stats_shooting',st_statistic, 'https://fbref.com/en/comps/9/shooting/Premier-League-Stats'),
    ('table#stats_passing',ps_statistic, 'https://fbref.com/en/comps/9/passing/Premier-League-Stats'),
    ('table#stats_gca',gca_statistic, 'https://fbref.com/en/comps/9/gca/Premier-League-Stats'),
    ('table#stats_defense',dfs_statistic, 'https://fbref.com/en/comps/9/defense/Premier-League-Stats'),
    ('table#stats_possession',pss_statistic, 'https://fbref.com/en/comps/9/possession/Premier-League-Stats'),
    ('table#stats_misc',mis_statistic, 'https://fbref.com/en/comps/9/misc/Premier-League-Stats')
]

dataframes = [df]
for table, target_statistic, url in targets:
    driver.get(url)
    rows = WebDriverWait(driver, 20).until(
        ec.presence_of_all_elements_located((By.CSS_SELECTOR, f'{table} tbody > tr:not(.thead)'))
    )
    df_target = get_statistic(target_statistic, rows)
